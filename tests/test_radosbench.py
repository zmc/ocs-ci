...
def run(**kw):
    ...
    log.info(f"Running radosbench task")
    ...
    out, err, ret = client.exec_command(
        cmd=rados_bench, check_ec=True,
        long_running=True, timeout=timeout
    )
    if ret:
    ...
    log.info(out)
    log.info(f"Finished radosbench")
    return ret
